...
}
{
  ...
  const writeBack = (
    absolutePath: string,
    content: ByteBuffer | null | undefined
  ) => {
    postMessage({ f: "WRITE", x: { path: absolutePath, content: content } });
  };
  const isDirIndicator = (absolutePath: string) =>
    `<title>Index of ${absolutePath}`;
  const isDir = (
    absolutePath: string,
  ...
  const rawReadHttpServer1 = (absolutePath: string): ByteBuffer | undefined => {
    const request = new XMLHttpRequest();
  ...
  const readFileSync = (absolutePath: string): ByteBuffer => {
    // - try vfs
    {
      if (absolutePath in env.fs) {
        return env.fs[absolutePath] === null
          ? //@ts-ignore
            zenfs.readFileSync(absolutePath).toString()
    ...
    // @ts-ignore
    // console.log(zenfs.readdirSync("/"))
    // - try server
    if (!("__NOHTTP" in env.fs)) {
      const result =
        rawReadHttpServer(absolutePath) || throwENOENT(absolutePath);
      const result1 =
        rawReadHttpServer1(absolutePath) || throwENOENT(absolutePath);
      if (isDir(absolutePath, result)) err("TODO: correct message");
      //@ts-ignore
      zenfs.writeFileSync(absolutePath, result1);
  ...
  const readDirSync = (absolutePath: string): string[] => {
    ...
    const envFsExists = Object.keys(env.fs).some((x) =>
      x.startsWith(absolutePath)
    );
    // known files?
    ...
    if (!("__NOHTTP" in env.fs)) {
      ...
      if (result !== undefined) {
        ...
        files.push(...matches);
      } else if (!envFsExists) throwENOENT(absolutePath);
    }
  ...
  selfAny.onmessage = function (msg: MessageEvent) {
    ...
    const nativesKeys = [
      ...
      "zlib.pretty",
      "internal/buffer",
      "internal/zlib/messages",
      "internal/zlib/utils-common",
      "internal/zlib/trees",
      "internal/zlib/inftrees",
      "internal/zlib/inffast",
      "internal/zlib/inflate",
      "internal/zlib/inffast",
      "internal/zlib/deflate",
      "internal/zlib/constants",
      "internal/zlib/adler32",
      "internal/zlib/crc32",
      "internal/zlib/binding",
    ...
    function ensureExists() {
      ensureDirectoryExists("/node");
      ensureDirectoryExists("/node/internal");
      ensureDirectoryExists("/node/internal/test");
      ensureDirectoryExists("/node/internal/streams");
      ensureDirectoryExists("/node/internal/process");
      ensureDirectoryExists("/node/internal/loader");
      ensureDirectoryExists("/node/internal/crypto");
      ensureDirectoryExists("/node/internal/cluster");
      ensureDirectoryExists("/node/internal/zlib");
    }
    async function LoadFS1() {
      ...
      const internalNodes = [
        ...
        "streams/legacy",
        "streams/destroy",
        // Add other internal keys as needed
      ];

      const fetchAndWrite = async (path: string) => {
        try {
          ...
          const fileContent = await response.text(); // or response.arrayBuffer() for binary files

          // Write the fetched content to ZenFS
          // @ts-ignore
          ensureExists()
          // @ts-ignore
          zenfs.writeFileSync(path, fileContent, { encoding: "utf8" });
          console.log(`Successfully wrote ${path} to ZenFS.`);
        } catch (error) {
          //@ts-ignore
      ...
      // Fetch and write from /node/*
      for (const node of baseNodes) {
        await fetchAndWrite(`/node/${node}.js`);
      }

      // Fetch and write from /node/internal/*
      for (const internalNode of internalNodes) {
        await fetchAndWrite(`/node/internal/${internalNode}.js`);
      }
    }
      ...
      async function fetchDirectory(url:string) {
        ...
        try {
          ...
          console.error(
            "Failed to load directory contents. and ",
            "Fetch error:",
            error
          );
          // alert();
    ...
    function displayFiles(
      //@ts-ignore
        ...
        contents.forEach((item) => {
          ...
          if (item.isDirectory) {
            saveToZenFS(normalizedPath, {}); // Save directory as an empty object
            // Recursively handle the contents of the directory
            if (item.contents) {
              displayFiles(item.contents, normalizedPath + "/");
            }
          } else {
      ...
      function saveToZenFS(filePath, fileContent) {
        ...
        pathParts.forEach((part, index) => {
          ...
          globalThis.console.log(part,index,normalizedPart)
          //@ts-ignore
          if (!current[normalizedPart]) {
            ...
            //       //@ts-ignore
            //       //@ts-ignore
            //       zenfs.writeFileSync(filePath, fileContent);
            //     } catch (error) {
      ...
      function normalizeName(name:string) {
        ...
        ensureExists()
        // Call the function to load all files
        LoadFS1();
    ...
    for (const nativesKey of nativesKeys)
      natives[nativesKey] = arr2str(
        readFileSync(`/node/${nativesKey}.js`) ||
          err(`missing native '${nativesKey}'`)
      );
    natives["config"] =
    ...
    const process = {
      ...
      _rawDebug: (x: any) =>
        postMessage({ f: "error", x: { f: "_rawDebug", x: x } }),
        ...
        switch (name) {
            ...
            return {
              ...
              setupBufferJS: (proto: any) => {
                ...
                proto.utf8Write = function (
                  ...
                  for (
                    var i = 0;
                    i < length && i < this.byteLength - offset;
                    ++i
                  )
          ...
          case "cares_wrap":
            return {
              ...
              getaddrinfo: function (
                ...
                addr_info_wrap.oncomplete({
                  0: 0,
                  1: [addr_info_wrap.hostname],
                });
                return 0; // = success
              },
            }; // TODO
          ...
          case "fs":
            ...
            const fstat = (
              ...
              // TODO
              if (req)
                nextTick(() => req.oncomplete(/*error, if one happened*/));
            };
            return {
              //@ts-ignore
              fs2: zenfs,
              ...
              lstat: (path: string, req?: FSReqWrap) => {
                try {
                  try {
                    ...
                    if (buffer) return fstat({ s: buffer, isDir: false }, req);
                  } catch {}
                  if (readDirSync(path))
                    return fstat({ s: new Uint8Array(0), isDir: true }, req);
              ...
              open: (
                ...
                return wrap<FileDescriptor>(() => {
                  if (flags === 0)
                    return { s: readFileSync(path), isDir: false };
                  if (flags === 2)
                    return { s: readFileSync(path), isDir: false };
                  if (flags === 266)
                    return { s: readFileSync(path), isDir: false };
    ...
    //@ts-ignore
    // fs2
    const bootstrapper = new ContextifyScript(
      natives["internal/bootstrap_node"],
      {
        displayErrors: true,
